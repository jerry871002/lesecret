...
def test_key_generation():
    ...
    assert len(key) == len(Fernet.generate_key()), f'Key length should be {len(Fernet.generate_key())} bytes'
    try:
...
def test_full_encode_decode_cycle(temporary_image):
    ...
    assert decrypted_message.decode(ENCODING) == message, 'Decoded and decrypted message should match the original'
